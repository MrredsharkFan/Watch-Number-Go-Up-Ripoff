...
function upgEffect(x) {
    ...
    return [l,new Decimal.pow(2,r)]
}

function upgCost(x) {
    return new Decimal.pow(1.3,x.pow(7/5)).times(8)
}
...
function barText(x) {
    ...
    var t = ""
    for (i in x) {
        t = t + `<div style="position: absolute; top: ${i * 3 + 35}%; height: 3%; width: 50%">${x[i]}/5 <span style="color: #555; font-size: 11px">(x${format(Decimal.pow(2, Decimal.pow(7, i)))} for each)</span><div style="z-index: -10000; position: absolute; top: 0%;height: 100%; width: ${x[i].times(20)}%; background-color: ${cl(i)}"></div></div>`
    }
    return t
}
